...
#have to remove the "lab8" from the command because here tells us that we are already in lab8
...
(1) each variable has its own column?  
  no, the different drugs are data as well
...
heartrate %>% 
  pivot_longer(-patient, #patient will not pivot
               names_to = "drug", #makes a new column name for the former variables to be moved to
               values_to = "heartrate" #values (from the old variables) moved to a new column titled heartrate
               )
```
...
the income should be a variable, not a classification for variables
