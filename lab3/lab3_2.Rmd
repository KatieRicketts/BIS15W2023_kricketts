...
#need to specify the path with the data/ at the front
#or you can put the entire location at the front: "/Users/ksricket/Desktop/BIS15W2023_kricketts-main/lab3/data/Gaeta_etal_CLC_data.csv"
```
...
```{r}
head(fish, n = 10)
#you can choose the number of rows it gives in the n = x
```
...
```{r}
names(fish)
#length is a name of a variable
```
...
```{r}
#you can only do one column at a time, use the $ to specify
table(mammals$order)
```
```{r}
