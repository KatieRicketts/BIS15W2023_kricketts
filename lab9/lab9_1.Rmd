...
ggplot(homerange, mapping=aes(x=log10.hra, y=log10.preymass))+
  geom_point(na.rm=T)+
  geom_smooth(method=lm, se=T)
#ggplot removes any rows that contain NAs when creating a plot
#adding the na.rm=T makes the error message go away
```
...
homerange %>% 
  ...
  geom_bar()
#don't have to specify y because bar plots use the count
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_col()+
  coord_flip() #automatically switches the x and y, so that all of the names of the species can be read
```
