...
fish %>%
  ...
  filter(lakeid == "AL")
#"take the fish data and pipe it into the select command, then feed it through the filter command"
```
...
fish_subset
```
unsure if the between() command will inlcude the bounds or not...
...
#^this doesn't work because you can't filter after that variable has been removed
fish %>%
  filter(fish_id == "300") %>%
  select(radii_length_mm, scalelength) %>%
  arrange(radii_length_mm)
...
```
Balaenopteridae Balaenoptera physalus has the oldest known life span (fin whale)
