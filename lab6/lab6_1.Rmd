...
```
you can also get min/max, standard dev, and count etc
...
```{r}
msleep %>% 
  summarize(n_genera_msleep = n_distinct(genus))
```
```{r}
...
```{r}
msleep %>% 
  summarize(min_sleep_total = mean(sleep_total),
...
# if there is one obsevation that has an NA, then no mean is produced
```
...
msleep %>% 
  ...
  select(order, genus, brainwt)
```
none of the whales (cetacea) have a measured brain weight, so no mean brainwt for cetacea
