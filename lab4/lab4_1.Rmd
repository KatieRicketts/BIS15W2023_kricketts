...
```
if only interested in lakeid and annnumber:
```{r}
select(fish, "lakeid", "annnumber")
#put lakeid and annnumber in quotes to tell R that they are variables, and not objects
#if no quotes are included, it can lead to errors later on
```
...
```
the mean returns as a negative (even though its a measurement of mass) because the data collectors used -999.00 instead of leaving a spot blank
...
```{r}
mammals_new <- rename(mammals, genus = "Genus", 
                               wean_mass = "wean mass", 
                               afr = "AFR", 
                               max_life = "max. life", 
...
```{r}
select(mammals_new, starts_with("mass"))
```

8. Select all of the columns that are of class `character`.  
```{r}
# select(mammals_new, ~is.character(.)) it's redundant to use both the ~ and . and it works without them:
