...
```
```{r}
homerange <- janitor::clean_names(homerange)
```
```{r}
names(homerange)
```
...
2. Now, add a fill color for each box by taxonomic class
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10_mass, fill=class))+
  geom_boxplot()
```
...
homerange %>% 
  ggplot(aes(x = log10_mass)) +
  geom_histogram(alpha = 0.7, color = "gray30", fill = "plum4", bins=20)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(fill="olivedrab4", color="black", alpha=0.5)+
...
homerange %>% 
  ggplot(aes(x=log10_hra))+
  geom_density(fill="olivedrab4", color="black", alpha=0.5)+
  labs(title="Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10_hra))+
  geom_histogram(aes(y=after_stat(density)), fill="olivedrab4", color="black", alpha=0.5)+
...
homerange %>% 
  select(log10_mass) %>% 
  summarise(min=min(log10_mass),
            max=max(log10_mass))
```

```{r}
summary(homerange$log10_mass)
```
...
```{r}
homerange %>% 
  ggplot(aes(x = mass_category, fill = trophic_guild)) +
...
```{r}
homerange <- homerange %>% 
  mutate(range_category=case_when(log10_hra<3.65 ~ "very_small",
                                  log10_hra>=3.65 & log10_hra<=4.59 ~ "small",
                                  log10_hra>=4.59 & log10_hra<=6.02 ~ "medium",
                                  log10_hra>=6.02 ~ "large"))
homerange
```
...
homerange %>% 
  ...
  labs(title="Observations by Range Cateogry",
       x="Range Category",
